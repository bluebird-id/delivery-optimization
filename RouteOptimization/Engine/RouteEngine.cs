...
[assembly: InternalsVisibleTo( "RouteOptimization.UnitTest" )]
namespace RouteOptimization.Engine
{
    public partial class RouteEngine
    {
        public List<Models.RouteModel> RouteSolution( List<Models.ShipmentModel> lShipment, List<Models.VehicleModel> lVehicles, IConfiguration configuration )
        {
            List<Models.LocationModel> routes = new List<Models.LocationModel>();
            long index = 0;
            var lShipmetDemand = GetShipmentDemands( lShipment );
            AppendLocationToRoutes( routes, lShipment );
            List<VehicleCapacity> lVehicleCapacity = GetVehicleCapacities( lVehicles );
            //vehicle location
            routes.Add( lVehicles[ 0 ].StartLocation );

            //assign pickup and delivery point
            var pickDelivery = GetPickUpPoints( lShipment );
            var lTimeWorkingVh = GetVehicleWorkingTime( lVehicles );
            var minDate = lTimeWorkingVh.Min();
            var maxDate = lTimeWorkingVh.Max();
